 ...
 * @author likui
 */
public class CopyOfInsertTxtByCon_GAC {
...
//			QueryESInSertTxt("D:\\cphm.txt", cphm, "", "", "2014-05-27 17:00:00", "2014-05-28 17:00:00", "D://多次违章车辆轨迹20140528//");
			...
			//海关缉查车辆轨迹
			QueryESInSertTxt("D:\\cphm.txt", "", "", "10200402,10100404,10100403,10300501,10100407,10100408,10300503,10300504,10300506," +
					"10100405,10100406,10300502,10300505", "2014-06-07 14:00:00", "2014-07-08 18:00:00", "D://车辆轨迹_深港//");
...
//			while ((str = bf.readLine()) != null) {//每个车牌号查询一次
				...
				file = new File(filePath+File.separator+"深港一日多次"+".csv");
				if(!file.exists()){
...
//						getPic(file.getParentFile().getAbsolutePath()+File.separator+tpid+".jpg",tpid);
//						
//						getPic(file.getParentFile().getAbsolutePath()+File.separator+"全景"+File.separator+tpid+".jpg",
//								tpid.substring(0,tpid.length()-1)+ "2");
...
//				query.must(termsQuery ("cphm1", cphid.split(",")));
				query.must(prefixQuery("cphm1", "粤Z"));
//			}
			// 车牌类型
			...
			if (jcdid != null && !"".equals(jcdid)) {
				if(jcdid.contains(",")){
					query.must(termsQuery("jcdid", jcdid.split(",")));
				}else{
