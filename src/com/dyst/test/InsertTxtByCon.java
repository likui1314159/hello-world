...
//			QueryESInSertTxt("D:\\cphm.txt", cphm, "", "", "2014-05-27 17:00:00", "2014-05-28 17:00:00", "D://多次违章车辆轨迹20140528//");
			...
			//海关缉查车辆轨迹
			QueryESInSertTxt("D:\\cphm2.txt", cphm, "", "", "2014-05-27 17:00:00", "2014-05-28 17:00:00", "D://海关缉查车辆轨迹20140528//");
...
//				}
	 ...
	 * @param filePath
	 */
	public static void QueryESInSertTxt(String txtFile, String hphm,String cplx ,String jcdid, String beginTime,String endTime,String filePath) {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		TransportClient clientTrans = null;
		ESClientManager ecclient = ESClientManager.getInstance();
//		Connection connection = null;
...
//		---end----------------
		...
		try {
			...
			while ((str = bf.readLine()) != null) {//每个车牌号查询一次
				...
				file = new File(filePath+File.separator+str+File.separator+str+".csv");
				if(!file.exists()){
...
//				System.out.println("存放路径"+filePath);
				...
				//----------------------------------------ES开始-----------------------
			    //生成查询query条件
				QueryBuilder query = getQueryByCon(str.trim(), jcdid, cplx, beginTime, endTime);// 获取查询Query
				...
				for(int j=0;j < count.intValue();j+=bulkcount){//每次查询输出一万条，
					...
					for (int i = 0; i < hits.getHits().length; i++) {
						...
						if(k<10){//每个车牌号如果有轨迹，需要调10张图片
							if(getPic(file.getParentFile().getAbsolutePath()+File.separator+tpid+".jpg",tpid)){
								k++;
							}
						}
...
//						bw.write(conent);
				...
				//----------------------------------------停车场开始-----------------------
				rs = st.executeQuery("select CPHM,CJSJ,TCCXKZ,TCCMC from JYXTCCCLXXCJ where (LRSJ " +
						"between to_date('" + beginTime + "','yyyy-MM-dd HH24:mi:ss') " +
								"and to_date('" + endTime + "','yyyy-MM-dd HH24:mi:ss')) " +
								"and CPHM = '" + str.trim() + "'");
				while(rs.next()){
					String conent = rs.getString("CPHM") + ","
						+ sdf.format(rs.getDate("CJSJ")) + ","
						+ rs.getString("TCCXKZ") + ","
						+ rs.getString("TCCMC");
						...
						bw.write(new String(conent.getBytes("utf-8"), "UTF-8"));
						bw.newLine();
						System.out.println("停车场正在插入....." + conent);
				}
				
				//----------------------------------------路边停车采集开始-----------------------
				rs = st.executeQuery("select t.CPHID,t.CJSJ,j.dlmc, j.ldmc,t.TPLJ from TCCCLXXCJ t, " +
						"clxxcjd j where j.dldm = t.dldm and j.lddm = t.lddm and (LRSJ " +
						"between to_date('" + beginTime + "','yyyy-MM-dd HH24:mi:ss') " +
								"and to_date('" + endTime + "','yyyy-MM-dd HH24:mi:ss')) " +
								"and CPHID = '" + str.trim() + "'");
				while(rs.next()){
					String tpid = rs.getString("TCCMC").substring(rs.getString("TCCMC").lastIndexOf("/")+1);
					String conent = rs.getString("CPHID") + ","
						+ sdf.format(rs.getDate("CJSJ")) + ","
						+ rs.getString("dlmc") + ","
						+ rs.getString("ldmc") + ",#" + tpid.substring(0, tpid.indexOf("."));
							
						//读取内容写文本文件
						bw.write(new String(conent.getBytes("utf-8"), "UTF-8"));
						bw.newLine();
						System.out.println("路边停车采集正在插入....." + conent);
						
						//拷贝图片
						downloadFile(file.getParentFile().getAbsolutePath() + 
								File.separator + tpid, "http://100.100.37.38:8080/tcctp/" + rs.getString("TCCMC"));
				}
