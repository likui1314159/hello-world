 ...
 * @author likui
 */
public class InsertTxtByCon_NTC {
...
//			QueryESInSertTxt("D:\\cphm.txt", cphm, "", "", "2014-05-27 17:00:00", "2014-05-28 17:00:00", "D://多次违章车辆轨迹20140528//");
			...
			//海关缉查车辆轨迹
			QueryESInSertTxt("D:\\cphm.txt", "", "2", "", "2014-07-07 14:00:00", "2014-07-07 18:00:00", "D://车辆轨迹_泥头车//");
 ...
 * @param filePath
 */
	public static void QueryESInSertTxt(String txtFile, String hphm,String cplx ,String jcdid, String beginTime,String endTime,
			String fileFolder) {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		TransportClient clientTrans = null;
		ESClientManager ecclient = ESClientManager.getInstance();
...
//			//读取txt文件的车牌号，每一个车牌号查询一次轨迹，写入指定的文件夹中
			...
			//数据写入文件
			String pathFile=fileFolder+File.separator+"轨迹信息"+".txt";
			int txtLine = 0;
			int countCph=0;
			String cphms="";
			while ((str = bf.readLine()) != null) {//每个车牌号查询一次
				System.out.println("执行行号："+txtLine++);
				...
				if(countCph<200){
					...
					//生成查询query条件
					query = getQueryByCon(cphms.trim(), jcdid, cplx, beginTime, endTime);// 获取查询Query
					countCph=0;
					cphms="";
				}
...
//									      .execute().actionGet();
//				Long count = (Long)response1.getHits().getTotalHits();//符合条件记录总数
//				System.out.println("count总记录数："+count);	
				int bulkcount = 10000;
//				int k = 0;
...
//						getPic(fileFolder+File.separator+"全景"+File.separator+tpid2+".jpg",tpid2,webClient);
						...
						writerTXT(pathFile, conent);
						//读取内容写文本文件
						System.out.println("....."+(i)+":.."+hits.getAt(i).getSource().get("cphm1"));
					}
					//for循环完后需要刷新
//				}
			}
